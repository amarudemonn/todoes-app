...
def index():  
  if request.method == "POST":
    ...
    if len(task) < 2:
      flash("Your task should be at least 2 characters long(like the word be)", category="error")
      return redirect(url_for("index"))
    ...
    if task.startswith(" ") or task.endswith(" "):
      flash("Your task should not start with spaces or end with spaces", category="error")
      return redirect(url_for("index"))
    ...
    tasks = Task.query.filter_by(user_id=session["user_id"]).with_entities(Task.task_todo, Task.is_completed).all()
...
def register():
  ...
  if request.method == "POST":
    ...
    if password.islower():
      flash("Password should contain at least 1 upper case character.", category="error")
      return redirect(url_for("register"))
